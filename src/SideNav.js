...
const SideNav = ({ onNewText }) => {
    const [open, setOpen] = useState(false);


    return(
            
        <div className={`bg-dark-white h-screen z-200 p-5 pt-8 ${open ? "w-72" : "w-20"} duration-300 sticky top-0`}>
        ...
        <div className={`rounded-full mt-20 border bg-max-dark-white items-center p-2 transition-all duration-300 ${open ? "inline-flex" : "flex justify-center"}`}>
            <BsPlusLg className="text-lg text-[#7e7f81] font-bold transition-all duration-300" onClick={onNewText}/>
            <div className={`overflow-hidden transition-all duration-300 ${open ? "max-w-full opacity-100 ml-2 pr-5" : "max-w-0 opacity-0"}`}>
                {open && (
                <button className="text-lg text-[#7e7f81] font-medium"  onClick={onNewText}>
                    New Text
                </button>
                )}
            </div>
